...
def getCampaignType(campaign) {
    new JsonSlurper().parseText(campaign).variants.find { it.variantType != "control"}.variantType
}

println getCampaignType(campaign1)

def isCouponCampaign(campaign) {
    new JsonSlurper().parseText(campaign).variants.any { it.variantType == "coupon" }
}
